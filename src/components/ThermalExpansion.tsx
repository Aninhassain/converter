...
const ThermalExpansion = () => {
  ...
  const [fromUnit, setFromUnit] = useState<string>('per-kelvin');
  const [toUnit, setToUnit] = useState<string>('per-celsius');

  const units = [
    { value: 'per-kelvin', label: 'Per Kelvin [K⁻¹]', factor: 1 },
    { value: 'per-celsius', label: 'Per Celsius [°C⁻¹]', factor: 1 },
    { value: 'per-fahrenheit', label: 'Per Fahrenheit [°F⁻¹]', factor: 0.555556 }
  ];
        ...
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-6">
            <div className="flex items-center justify-center gap-3 mb-4">
              ...
              <h1 className="text-3xl font-bold text-gray-900">Thermal Expansion Converter</h1>
            </div>
            <p className="text-gray-600">Convert thermal expansion coefficients between different temperature units.</p>
          </div>
