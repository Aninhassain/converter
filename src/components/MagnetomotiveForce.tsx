...
const MagnetomotiveForce = () => {
  ...
  const [fromUnit, setFromUnit] = useState<string>('ampere-turn');
  const [toUnit, setToUnit] = useState<string>('gilbert');

  // 1 gilbert = 10/(4*pi) ampere-turn
  const GILBERT_TO_AT = 10 / (4 * Math.PI);

  const units = [
    { value: 'ampere-turn', label: 'Ampere-turn [A·turn]', factor: 1 },
    { value: 'gilbert', label: 'Gilbert [Gi]', factor: GILBERT_TO_AT }
  ];
        ...
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-6">
            <div className="flex items-center justify-center gap-3 mb-4">
              ...
              <h1 className="text-3xl font-bold text-gray-900">Magnetomotive Force Converter</h1>
            </div>
